...
int main(int argc, char const *argv[]) {
	...
	socklen_t s1=sizeof(local);
		...
		if(getsockname(_sfd,(struct sockaddr *)&local,&s1)<0)
			perror("NULL");
		printf(" %d   %s",ntohs(remote.sin_port),inet_ntoa(local.sin_addr));

		if(getpeername(_sfd,(struct sockaddr *)&remote,&s2)<0)
			perror("NULL");
		printf(" %d   %s",ntohs(remote.sin_port),inet_ntoa(remote.sin_addr));
        nClient++;
